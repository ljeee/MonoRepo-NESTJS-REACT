...
export class InventarioBebidasService {
	 ...
	 */
	async deductForOrden(items: Array<{varianteId: number | null; cantidad: number}>, manager?: EntityManager) {
		if (!items.length) return;

		const varianteIds = items.map((i) => i.varianteId).filter((v): v is number => v !== null);
