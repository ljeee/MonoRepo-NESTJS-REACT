...
export class EstadisticasController {
	...
	productosTop(@Query('from') from: string, @Query('to') to: string, @Query('limit') limit?: string) {
	...
	saboresTop(@Query('from') from: string, @Query('to') to: string, @Query('limit') limit?: string) {
	...
	@ApiQuery({name: 'to', required: true})
	@ApiQuery({name: 'limit', required: false})
	variantesTop(@Query('from') from: string, @Query('to') to: string, @Query('limit') limit?: string) {
	...
	ventasPorHora(@Query('fecha') fecha?: string, @Query('from') from?: string, @Query('to') to?: string) {
	...
	@ApiQuery({name: 'from', required: true})
	@ApiQuery({name: 'to', required: true})
	ventasPorDia(@Query('from') from: string, @Query('to') to: string) {
	...
	@ApiQuery({name: 'from', required: true})
	@ApiQuery({name: 'to', required: true})
	metodosPago(@Query('from') from: string, @Query('to') to: string) {
	...
	@ApiQuery({name: 'from', required: true})
	@ApiQuery({name: 'to', required: true})
	resumenPeriodo(@Query('from') from: string, @Query('to') to: string) {
	...
	clientesFrecuentes(@Query('limit') limit?: string, @Query('from') from?: string, @Query('to') to?: string) {
		return this.service.clientesFrecuentes(limit ? Number(limit) : 10, from, to);
	}

	@Get('domiciliarios')
	@ApiOperation({summary: 'Estadísticas por domiciliario'})
	domiciliariosStats(@Query('from') from: string, @Query('to') to: string) {
