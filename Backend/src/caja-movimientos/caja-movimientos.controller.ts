...
export class CajaMovimientosController {
    ...
    apertura(@Body() body: { denominaciones: DenominacionesMap; descripcion?: string; fecha?: string; cajaOrigen?: CajaOrigen }) {
        return this.service.registrarApertura(body);
    }

    @Post('ajuste')
    @Roles(Role.Admin, Role.Cajero)
    ajuste(@Body() body: { tipo: 'entrada' | 'salida' | 'cambio'; denominaciones: DenominacionesMap; descripcion: string; fecha?: string; cajaOrigen?: CajaOrigen }) {
