...
describe('OrdenesProductosService', () => {
	...
	beforeEach(async () => {
		...
		const module: TestingModule = await Test.createTestingModule({
			providers: [OrdenesProductosService, {provide: getRepositoryToken(OrdenesProductos), useValue: mockRepo}],
		}).compile();
	...
	describe('findAll', () => {
		it('retorna los productos de ordenes con relaciones', async () => {
			...
			expect(mockRepo.find).toHaveBeenCalledWith(expect.objectContaining({relations: ['orden', 'productoObj']}));
		});
		...
		it('aplica paginación correctamente', async () => {
			...
			expect(mockRepo.find).toHaveBeenCalledWith(expect.objectContaining({take: 50, skip: 50}));
		});
	});
