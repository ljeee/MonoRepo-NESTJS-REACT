...
describe('PizzaSaboresService', () => {
	...
	beforeEach(async () => {
		...
		const module: TestingModule = await Test.createTestingModule({
			providers: [PizzaSaboresService, {provide: getRepositoryToken(PizzaSabor), useValue: mockRepo}],
		}).compile();
	...
	describe('findAll', () => {
		it('retorna todos los sabores ordenados', async () => {
			...
			expect(mockRepo.find).toHaveBeenCalledWith(expect.objectContaining({order: {tipo: 'ASC', nombre: 'ASC'}}));
		});
	});
