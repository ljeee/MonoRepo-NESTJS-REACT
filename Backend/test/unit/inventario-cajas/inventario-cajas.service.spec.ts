...
import { InventarioCajas, InventarioCajasMovimiento } from '../../../src/inventario-cajas/esquemas/inventario-cajas.entity';
...
describe('InventarioCajasService', () => {
	...
	describe('ajustar', () => {
		...
		it('infiere tipo "entrada" cuando delta es positivo', async () => {
			...
			expect(mockMovimientosRepo.create).toHaveBeenCalledWith(
	...
	describe('getMovimientos', () => {
		it('retorna los movimientos con cajaNombre', async () => {
			const movimientos = [
				{ id: 1, cajaId: 1, caja: { nombre: 'Caja Grande' }, delta: -2, cantidadResultante: 8, tipo: 'salida', nota: 'Orden #5', creadoEn: new Date() },
			];
		...
		it('usa "Caja eliminada" cuando la caja ya no existe', async () => {
			const movimientos = [
				{ id: 2, cajaId: 5, caja: null, delta: -1, cantidadResultante: 0, tipo: 'salida', nota: null, creadoEn: new Date() },
			];
			mockMovimientosRepo.find.mockResolvedValue(movimientos);
	...
	describe('descontarCajasParaOrden', () => {
		...
		it('omite productos que no llevan caja', async () => {
			await service.descontarCajasParaOrden(
				[{ varianteNombre: 'Normal', tipoProducto: 'bebida', cantidad: 3 }],
				8,
			);
