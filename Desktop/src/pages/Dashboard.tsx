...
export function DashboardPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            // Counts
            setPendientes(ords.filter(o => o.estadoOrden === 'pendiente').length);
            setCompletadas(ords.filter(o => o.estadoOrden === 'completado').length);
            setSinPagar(facts.filter(f => f.estado === 'pendiente').length);
        } catch (err) {
                                ...
                                <div key={v.hora} className="mini-hour-col">
                                    <div className="mini-hour-track">
                                        <div
                                            ...
                                            style={{ height: `${(v.cantidad / maxHora) * 100}%` }}
                                        />
                                    </div>
                        ...
                        {ordenes.map(o => (
                            <Link key={o.ordenId} to={`/ordenes/${o.ordenId}`} className="recent-order-row" style={{ textDecoration: 'none', color: 'inherit' }}>
                                ...
                                <div className="recent-order-detail">
                                    <div className="recent-order-name">{o.nombreCliente || 'Sin nombre'}</div>
