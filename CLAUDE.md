...
**RiderApp — estabilidad**: NUNCA llamar `Location.startLocationUpdatesAsync` sin `ACCESS_BACKGROUND_LOCATION` concedido — en Android 14 lanza un `SecurityException` fatal (bug histórico); `useLocationTracking` en `App.tsx` hace `return` temprano si el permiso de background está denegado y todo el flujo está blindado con try/catch. La app está envuelta en un `ErrorBoundary` propio (`RiderApp/src/components/ErrorBoundary.tsx`, RN puro, no usa `@/src/shared/tw`) que atrapa errores de render y muestra una pantalla "Reintentar" en vez de crashear a nativo. La background task se registra en `RiderApp/src/tasks/locationTask.ts`, importada en `index.js` **antes** de `App` (para el modo headless de Android).
