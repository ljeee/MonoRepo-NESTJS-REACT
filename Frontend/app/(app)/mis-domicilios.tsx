...
import { useToast, formatCurrency, formatDate } from '@monorepo/shared';
...
export default function MisDomiciliosScreen() {
    ...
    const fetchDomicilios = useCallback(async () => {
        try {
            ...
            console.error(error);
            showToast('Error al cargar tus domicilios', 'error');
        } finally {
            setLoading(false);
            setRefreshing(false);
        }
    ...
    if (loading) return <PageContainer><ListSkeleton count={4} /></PageContainer>;
...
function DomicilioCard({ item, onComplete, isCompleted = false }: { item: any; onComplete?: () => void; isCompleted?: boolean }) {
    const ec = getEstadoColor(item.estadoDomicilio || 'pendiente');
